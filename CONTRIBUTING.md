...
11. [Lobes: memory, policy, and evals](#lobes-memory-policy-and-evals)
...
- Use the issue templates: **Bug report**, **Feature request**, or **Lobe request**.
...
| `<agent>` | `orchestrator`, `improver`, or `lobe-<lobe-name>` |
...
agent/orchestrator/feat/55-release-notes-lobe
agent/lobe-backlog-refiner/fix/60-clarify-dor-skill
```
...
| Type | Use for |
|---|---|
| `feat` | A new capability for users or lobes |
| `fix` | A bug fix |
...
> **Memory changes** to a lobe are `feat` (new behavior) or `fix` (corrected behavior), scoped to the lobe — because memory *is* behavior.
...
| Scope | Area |
|---|---|
| `<lobe-name>` | Any directory under `lobes/`, e.g. `backlog-refiner` |
...
Scopes for lobes and adapters are discovered automatically from the directory names, so adding a lobe adds its scope.
...
- For lobe changes, look at the **eval before/after**, not only the text.
...
## Lobes: memory, policy, and evals

- Follow the lobe template in ARCHITECTURE.md §5 and the structure in [`lobes/README.md`](lobes/README.md).
...
- A change to a lobe's behavior or memory must add or update eval cases that demonstrate it (enforced once the eval harness exists).
- A change to a lobe's policy is safety-critical.
...
| `memory` | Automatically (labeler) | Changes lobe memory |
...
| `bug`, `enhancement`, `lobe-request` | Issue templates | Issue type |
| `needs-adr` | Manually | A decision must be recorded before merging |
| `agent:orchestrator`, `agent:improver`, `agent:lobe` | By the agent | Authored by an autonomous agent |
