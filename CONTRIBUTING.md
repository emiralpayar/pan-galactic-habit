...
- **Bots and agents may open PRs; they may never approve or merge them.**
...
**Allowed exceptions** (generated by tools): `dependabot/…`, GitHub's `revert-…`, and the Claude GitHub app's `claude/…` branches.
...
- **Bots and agents cannot approve.** A PR authored by an agent needs a human approval; a PR authored by a human in a Claude Code session is still that human's PR and needs a *different* human to approve.
...
|---|---|---|
| Local git hooks | `.githooks/` (after `make setup`) | No commits on `main`, branch names, commit messages, no pushes to `main` |
| Claude Code hook | `.claude/hooks/guard-git.sh` | Same, plus no `--no-verify`, force-push, PR merge/approval, or protection changes by the agent |
...
| GitHub ruleset | `.github/rulesets/protect-main.json` | PR-only `main`, approvals, required checks, squash only, linear history, no force-push or deletion — with **no bypass actors** |
