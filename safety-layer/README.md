...
The deterministic (non-LLM) engine that gates every write to an external system (ARCHITECTURE.md §2, §5.2). The engine is shared; what each lobe may write is defined by that lobe's `policy/`.
...
5. **No system-specific or lobe-specific rules** in the engine — those belong in adapters and policies.
