...
- **What it can write.** Files inside the checkout: the workflow runs the action with `--permission-mode acceptEdits`. Without a permission mode every write waits for an approval nobody can give in a headless run, and Claude can only read and comment. Writes outside the checkout are still refused.
- **Where its permissions live.** In `claude_args` in the workflow, never in `.claude/settings.json`: Claude Code ignores a project settings file's `permissions.allow` entries until the workspace is trusted through its interactive dialog, and a runner is never trusted. `deny` entries and the workflow's `--disallowedTools` are unaffected, so the guardrails still hold.
