...
- **What it can write.** Files inside the checkout: the workflow's `settings` input sets `permissions.defaultMode` to `acceptEdits`. Without a permission mode every write waits for an approval nobody can give in a headless run, and Claude can only read and comment. Writes outside the checkout are still refused.
- **Where its permissions live.** In the workflow — the `settings` input and `claude_args` — never in `.claude/settings.json`. Claude Code ignores a project settings file's `permissions.allow` entries until the workspace is trusted through its interactive dialog, and a runner is never trusted; the `settings` input lands in `~/.claude/settings.json`, which is not gated that way. `deny` entries and the workflow's `--disallowedTools` are unaffected from every source, so the guardrails still hold. The action does not forward a `--permission-mode` flag, which is why the mode is set through the input.
