...
Phase 1, in progress: build the Backlog Refiner without the habit template (ARCHITECTURE.md §12), in Python (ADR 0003) on the Agent Runtime (ADR 0004), with a REST-based Azure DevOps adapter (ADR 0005). Implemented so far: the Safety Policy loader and per-call and per-session write budgets (`safety-layer/core`), the Backlog Refiner's policy, and the Azure DevOps adapter's work item reads. The rest of `habits/`, `orchestrator/`, `improver/`, and `interface/` contains only READMEs and placeholders. Progress is tracked in #13.
