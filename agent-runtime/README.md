...
The model sees the Tool Surface and nothing else: no shell, files, web, or MCP server.
...
- Build the environment the SDK runs with: exactly one supported credential, no unnamed endpoint override, and no access to the user's own Claude settings.
...
4. **No ambiguous credentials.** More than one Claude credential, an unsupported authentication variable, or an endpoint override the committed configuration does not name is refused.
...
The SDK merges the environment it is given onto the parent's and cannot remove variables from it.
The backend therefore refuses to start when the parent sets an ambiguous or unsupported variable, and overrides every credential, endpoint, and provider variable it does not use with an empty value.
The CLI reads its settings from an empty temporary directory, not the user's.
...
The CLI reports its tool list before the first model turn, so the test needs no credential and makes no model call.
...
- Adding a tool to a habit's Tool Surface belongs to the habit, not here; a tool that writes to an external system does not belong on a Tool Surface at all.
